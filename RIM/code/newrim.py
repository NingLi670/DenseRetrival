...
class Base(object):
    def __init__(self, feature_size, eb_dim, s_num, c_num, label_num, pred_mode):
        ...
        with tf.name_scope('inputs'):
            self.search_res_ph = tf.placeholder(tf.float32, [None, s_num, c_num * eb_dim], name='search_res_ph')
            ...
            self.target_ph = tf.placeholder(tf.float32, [None, c_num * eb_dim], name='target_ph')
        ...
        with tf.variable_scope('embedding', reuse=tf.AUTO_REUSE):
            ...
            self.label_emb_mtx = tf.get_variable('label_emb_mtx', [label_num, eb_dim],
        ...
        self.search_res_label = tf.nn.embedding_lookup(self.label_emb_mtx,
                                                       self.search_res_label_ph)  # [batch_size, s_num, eb_dim]
        self.search_res = tf.reshape(self.search_res_ph, [-1, s_num, c_num * eb_dim])
        self.target = tf.reshape(self.target_ph, [-1, c_num * eb_dim])
