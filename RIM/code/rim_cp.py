...
class Base(object):
    ...
    def embed(self, sess, batch_data, l2_norm):
        search_res, target, search_res_label = sess.run(
            [self.search_res, self.target, self.search_res_label], feed_dict={
                self.search_res_ph: batch_data[0],
                self.search_res_label_ph: batch_data[1],
                self.search_res_len_ph: batch_data[2],
                self.target_ph: batch_data[3],
...
class RIM(Base):
    ...
    def save_embedding(self, sess, path):
        emb = sess.run(self.emb_mtx)
        np.save(path, emb)
