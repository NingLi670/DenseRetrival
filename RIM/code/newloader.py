...
class DataloaderRIM(object):
    def __init__(self, 
        ...
        with open(target_label_file) as f:
            self.target = f.readlines()
            self.labels = [int(l[:-1].split(',')[-1]) for l in self.target]
        ...
        with open(search_res_label_file) as f:
            self.search_res_label = f.readlines()        
        # with open(search_pool_file,'rb') as f:
        #     self.search_pool = pkl.load(f).tolist()
        #     self.search_pool.append([0 for i in range(80)])

        self.search_pool=[]
        for i in range(172):
            path="/NAS2020/Share/lining/rim_data/tmall/emb_search/emb_search"+str(i)+".pkl"
            with open(path,'rb') as f:
                self.search_pool.append(pkl.load(f))
            print('loaded',i) 
    ...
    def __next__(self):
        ...
        search_res_label_batch = [l + [0] * (self.s_num - len(l)) for l in search_res_label_batch] #change
        search_res_label_batch=[l[:self.s_num] for l in search_res_label_batch] #change
        ...
        for search_res_linenos in search_res_col_batch:
            ...
            for lineno in search_res_linenos:
                #search_res_s.append(self.search_pool[lineno]) #change
                search_res_s.append(self.search_pool[lineno//400000][lineno%400000])
                
            #change
            if len(search_res_s)>self.s_num:
                search_res_s=search_res_s[:self.s_num]
